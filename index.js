async function fetchRandomBreeds() {
  try {
    ...
    const breedsResponse = await fetch(
      "https://api.thecatapi.com/v1/breeds?api_key=live_I2qywVN5qig2F4dQDfd3WmqGKojJpbYIV2WRWu6W4Ds2ZaaxIkAWUTKcsNjTTGi6"
    );
    ...
    const breedImagePromises = selectedBreeds.map(async (breed) => {
      const imageResponse = await fetch(
        `https://api.thecatapi.com/v1/images/search?limit=1&breed_ids=${breed.id}&api_key=live_I2qywVN5qig2F4dQDfd3WmqGKojJpbYIV2WRWu6W4Ds2ZaaxIkAWUTKcsNjTTGi6`
      );
      //check if network request was successful
      if (!imageResponse.ok) {
